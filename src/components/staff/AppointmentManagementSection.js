...
const AppointmentManagementSection = () => {
    ...
    const verifyAppointment = async (appointmentId) => {
        // Đặt trạng thái loading cho nút xác nhận cụ thể
        setActionLoading(prev => ({ ...prev, [appointmentId]: true }));

        try {
            // Gọi API với phương thức PUT và appointmentId
                        ...
                        {appointments.length > 0 ? (
                            appointments.map((appointment) => (
                                <tr key={appointment.appointmentId}>
                                    ...
                                    {/* <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${appointment.isPaid ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
